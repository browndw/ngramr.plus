...
#' walk_data <- google_ngram(c("walk", "walks", "walked"), variety = "gb", by = "decade")
#' head(walk_data)
#' 
...
google_ngram <- function(word_forms, variety=c("eng", "gb", "us"), by=c("year", "decade")){
  ...
  if(any(stringr::str_detect(substring(gram, 1, 1), c("\u00df", "\u00e6", "\u00f0", "\u00f8", "\u0142", "\u0153", "\u0131", "\u0192", "\u00fe", "\u0225", "\u0259", "\u0127", "\u014b", "\u00aa", "\u00ba", "\u0263", "\u0111", "\u0133", "\u0254", "\u021d", "\u2170", "\u028a", "\u028c", "\u0294", "\u025b", "\u0261", "\u024b", "\u2171", "\u0283", "\u0207", "\u0251", "\u2172")))) gram <- "other"
  ...
  message("Accessing repository. For larger ones (e.g., ngrams containing 2 or more words) this may take a few minutes. A progress bar should appear shortly...")
  
  # Check internet connectivity first
  if (!curl::has_internet()) {
    stop("No internet connection detected. Please check your network connection and try again.")
  }
  ...
  all_grams <- tryCatch({
    suppressWarnings(readr::read_tsv_chunked(repo, col_names = FALSE, 
    ...
    if (grepl("HTTP error 404|Not Found", e$message, ignore.case = TRUE)) {
      stop("The requested Google Books ngram file was not found. This may occur if:\n",
    ...
    } else if (grepl("timeout|connection|network", e$message, ignore.case = TRUE)) {
      ...
      stop("HTTP error while accessing Google Books data:\n",
           "  ", e$message, "\n",
           "  This may be a temporary server issue. Please try again later.", call. = FALSE)
    } else {
      stop("Error downloading Google Books ngram data:\n",
           "  ", e$message, "\n",
           "  Please check your internet connection and try again.", call. = FALSE)
    }
  })
  
  # Check if we got any data
  if (is.null(all_grams) || nrow(all_grams) == 0) {
    warning("No data found for the specified word form(s): ", paste(word_forms, collapse = ", "), 
            ". This could mean:\n",
    ...
    return(data.frame(Year = integer(0), AF = numeric(0), Total = numeric(0), Per_10.6 = numeric(0)))
  }
